...
<svg width="{self.config.width}" height="{self.config.height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="50" y="540" font-family="{self.config.font_family}" font-size="{self.config.font_size_caption}"
    ...
    def generate_radial_chart(self, data: Dict[str, float], title: str) -> str:
        ...
        # Простая SVG радиальная диаграмма
  ...
  <text x="200" y="30" text-anchor="middle" font-family="{self.config.font_family}"
        font-size="{self.config.font_size_subtitle}" fill="{self.config.primary_color}">{title}</text>'''

        # Круги для шкалы
        for i in range(1, 6):
            r = radius * i / 5
            svg += f'<circle cx="{center_x}" cy="{center_y}" r="{r}" fill="none" stroke="{self.config.muted_color}" stroke-width="1" opacity="0.3"/>'

        # Радиальные линии
        angle_step = 360 / len(data)
        for i, (label, value) in enumerate(data.items()):
            angle = (i * angle_step - 90) * 3.14159 / 180  # в радианы
            x1 = center_x + radius * 0.1 * value / 100 * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)
            y1 = center_y + radius * 0.1 * value / 100 * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)
            x2 = center_x + radius * value / 100 * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)
            y2 = center_y + radius * value / 100 * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)

            # Линия от центра
            svg += f'<line x1="{center_x}" y1="{center_y}" x2="{x2}" y2="{y2}" stroke="{self.config.primary_color}" stroke-width="2"/>'

            # Метка
            label_x = center_x + (radius + 30) * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)
            label_y = center_y + (radius + 30) * (len(label) % 2 * 0.5 + 0.5) * (1 if i % 2 == 0 else -1)
            svg += f'<text x="{label_x}" y="{label_y}" text-anchor="middle" font-family="{self.config.font_family}" font-size="{self.config.font_size_caption}" fill="{self.config.text_color}">{label}</text>'
