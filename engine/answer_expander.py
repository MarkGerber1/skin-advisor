...
💡 **ГЛАВНЫЙ СОВЕТ**: {self._get_main_tip(profile)}"""
    ...
    def _analyze_color_profile(self, profile: UserProfile) -> str:
        ...
        if profile.season:
            season_name = profile.season.value.title() if hasattr(profile.season, 'value') else str(profile.season).title()
            undertone_desc = ""
            ...
            undertone_val = profile.undertone.value if hasattr(profile.undertone, 'value') else profile.undertone
    ...
    def _analyze_skin_profile(self, profile: UserProfile) -> str:
        ...
        if profile.sensitivity:
            sens_val = profile.sensitivity.value if hasattr(profile.sensitivity, 'value') else profile.sensitivity
    ...
    def _get_key_warnings(self, profile: UserProfile, products: List[Product]) -> str:
        ...
        if profile.sensitivity and (
            (hasattr(profile.sensitivity, 'value') and profile.sensitivity.value == "high") or 
    ...
    def _get_main_tip(self, profile: UserProfile) -> str:
        ...
        skin_type_val = profile.skin_type.value if hasattr(profile.skin_type, 'value') else profile.skin_type
    ...
    def _generate_color_analysis(self, profile: UserProfile) -> str:
        ...
        if profile.contrast:
            contrast_val = profile.contrast.value if hasattr(profile.contrast, 'value') else profile.contrast
            analysis += f"\n**Контрастность**: {contrast_val}"
            
        if profile.eye_color:
            eye_val = profile.eye_color.value if hasattr(profile.eye_color, 'value') else profile.eye_color
    ...
    def _generate_skin_analysis(self, profile: UserProfile) -> str:
        ...
        if profile.fitzpatrick:
            fitz_val = profile.fitzpatrick.value if hasattr(profile.fitzpatrick, 'value') else profile.fitzpatrick
            fitz_descriptions = {
            ...
            analysis += f"**Фототип по Фицпатрику**: {fitz_val} — {fitz_descriptions.get(fitz_val, '')}\n\n"
        ...
        if profile.skin_type:
            skin_val = profile.skin_type.value if hasattr(profile.skin_type, 'value') else profile.skin_type
    ...
    def _generate_pro_tips(self, profile: UserProfile) -> str:
        ...
        sensitivity_val = profile.sensitivity.value if hasattr(profile.sensitivity, 'value') else profile.sensitivity
        skin_type_val = profile.skin_type.value if hasattr(profile.skin_type, 'value') else profile.skin_type
        season_val = profile.season.value if hasattr(profile.season, 'value') else profile.season
