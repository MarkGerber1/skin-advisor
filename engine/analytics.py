...
class AnalyticsTracker:
    ...
    def user_started_test(self, user_id: int, test_type: str, session_id: Optional[str] = None) -> None:
        """Пользователь начал тест (palette|skin)"""
        self.emit("user_started_test", user_id, {
            "test_type": test_type,
            "started_at": time.time()
        }, session_id)
    
    def user_completed_test(self, user_id: int, test_type: str, 
    ...
    def recommendations_viewed(self, user_id: int, branch: str, products_count: int = 0,
                              session_id: Optional[str] = None) -> None:
        """Пользователь просмотрел рекомендации (makeup|skincare)"""
        self.emit("recommendations_viewed", user_id, {
            "branch": branch,
            "products_count": products_count,
            "viewed_at": time.time()
        }, session_id)
    
    def product_added_to_cart(self, user_id: int, product_id: str, 
    ...
    def cart_viewed(self, user_id: int, items_count: int, total_value: float, 
                   currency: str = "RUB", session_id: Optional[str] = None) -> None:
        """Пользователь просмотрел корзину"""
        self.emit("cart_viewed", user_id, {
    ...
    def cart_item_updated(self, user_id: int, product_id: str, 
                         variant_id: Optional[str] = None,
                         qty_before: int = 0, qty_after: int = 0,
                         session_id: Optional[str] = None) -> None:
    ...
    def cart_item_removed(self, user_id: int, product_id: str,
                         variant_id: Optional[str] = None,
                         session_id: Optional[str] = None) -> None:
    ...
    def checkout_clicked(self, user_id: int, items_count: int, total_value: float,
                        currency: str = "RUB", session_id: Optional[str] = None) -> None:
        """Пользователь нажал на оформление заказа"""
        self.emit("checkout_clicked", user_id, {
            "items_count": items_count,
            "total_value": total_value,
            "currency": currency,
    ...
    def external_checkout_opened(self, user_id: int, partner: str,
    ...
    def error_shown(self, user_id: int, error_code: str, place: str,
    ...
    def page_viewed(self, user_id: int, page_name: str, 
                   source: Optional[str] = None,
                   session_id: Optional[str] = None) -> None:
    ...
    def get_events_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        # Расчет конверсий
        test_completion_rate = (funnel_events["test_completions"] / funnel_events["test_starts"]) if funnel_events["test_starts"] > 0 else 0
        cart_conversion_rate = (funnel_events["cart_additions"] / funnel_events["recommendations_views"]) if funnel_events["recommendations_views"] > 0 else 0
        checkout_conversion_rate = (funnel_events["checkout_clicks"] / funnel_events["cart_views"]) if funnel_events["cart_views"] > 0 else 0
    ...
    def _integrate_with_business_metrics(self, event: AnalyticsEvent) -> None:
        ...
        try:
            ...
            if event.event_type == "product_added_to_cart":
                ...
                self.metrics_tracker.track_event("cart_add_success", event.user_id, {
...
def emit_event(event_type: str, user_id: int, payload: Dict[str, Any] = None, 
               session_id: Optional[str] = None) -> None:
...
def track_user_completed_test(user_id: int, test_type: str, 
                             duration_seconds: Optional[float] = None,
                             session_id: Optional[str] = None) -> None:
...
def track_recommendations_viewed(user_id: int, branch: str, products_count: int = 0,
                               session_id: Optional[str] = None) -> None:
...
if __name__ == "__main__":
    ...
    tracker.product_added_to_cart(
        user_id, "product_123", "shade_01", 
        "goldapple", 2500.0, "foundation", session_id
    )
