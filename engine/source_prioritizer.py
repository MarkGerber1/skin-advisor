...
class SourcePrioritizer:
    ...
    def _init_sources(self) -> Dict[str, SourceInfo]:
        ...
        golden_apple_domains = [
            "goldapple.ru", "goldenappletree.ru", "золотоеяблочко.рф"
        ]
        for domain in golden_apple_domains:
            sources[domain] = SourceInfo(
                domain=domain,
                priority=1,
                category="golden_apple", 
                name="Золотое Яблоко"
            )
    ...
    def sort_products_by_source_priority(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def group_by_source_category(self, products: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
