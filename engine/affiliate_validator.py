...
class AffiliateManager:
        ...
        self.affiliate_configs = {
            ...
            "ru_marketplace": {
                ...
                "partner_code": getattr(
                    self.settings, "ru_marketplace_partner_code", "BEAUTYCARE_MP"
                ),
    ...
    def track_external_checkout_opened(
        self, partner: str, url: str, items_count: Optional[int] = None
    ):
        ...
        """
        payload = {"partner": partner, "url": url, "items_count": items_count}
...
class AffiliateValidator:
    ...
    def validate_url(self, original_url: str, affiliate_url: Optional[str]) -> AffiliateCheckResult:
        ...
        try:
            ...
            if affiliate_tag and affiliate_tag != self.expected_affiliate_tag:
                issues.append(
                    f"Неожиданный affiliate tag: {affiliate_tag} (ожидался: {self.expected_affiliate_tag})"
                )
    ...
    def validate_selection_results(
        self, selection_results: Dict[str, Any]
    ) -> Dict[str, List[AffiliateCheckResult]]:
    ...
    def get_monetization_report(
        self, validation_results: Dict[str, List[AffiliateCheckResult]]
    ) -> Dict[str, Any]:
            ...
            for result in results:
                ...
                if result.is_valid and result.has_affiliate:
                    ...
                    issues_summary.append(
                        f"{category}: {getattr(result, 'product_id', 'unknown')} - нет affiliate"
                    )
                else:
                    broken_links += 1
                    issues_summary.append(
                        f"{category}: {getattr(result, 'product_id', 'unknown')} - битая ссылка"
                    )
...
def test_affiliate_manager():
    ...
    full_url = manager.get_affiliate_url(
        "https://goldapple.ru/product", "goldapple", "recommendation"
    )
