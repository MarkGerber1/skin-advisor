...
def _read_yaml(path: Path) -> Any:
	if yaml is None:
		raise RuntimeError("PyYAML не установлен. Установите pyyaml или используйте JSON каталог.")
	with path.open("r", encoding="utf-8") as f:
...
def _wrap_deeplink(url: str, partner_code: Optional[str], deeplink_cfg: Optional[DeeplinkConfig], redirect_base: Optional[str]) -> str:
	final_url = url
	subid = partner_code or ""
	if redirect_base:
		from urllib.parse import quote
		return f"{redirect_base.rstrip('/')}/r?url={quote(final_url)}&subid={quote(subid)}"
	if deeplink_cfg and deeplink_cfg.network.lower() != "none":
		...
		if net == "custom" and deeplink_cfg.custom_template:
			return deeplink_cfg.custom_template.replace("{url}", final_url).replace("{subid}", subid)
...
def _category_for_routines(category: str) -> Optional[str]:
	...
	if cat in ("маска", "mask", "пилинг", "peel"):
		return "weekly"
	# декоративка
	if cat in ("тон", "foundation", "powder", "blush", "bronzer", "concealer", "lip", "губы"):
		return "makeup"
	return None
...
def _build_product_card(product: Dict[str, Any], policy: PricingPolicy, partner_code: Optional[str], deeplink_cfg: Optional[DeeplinkConfig], redirect_base: Optional[str]) -> Dict[str, Any]:
	brand = product.get("brand", "")
...
def select_products(
	...
	summary = {
		...
		"disclaimer": user_profile.get("disclaimer") or "Информация носит образовательный характер.",
	}
	result: Dict[str, Any] = {"summary": summary, "products": picked, "routines": routines}
	if include_makeup:
