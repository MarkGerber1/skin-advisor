    ...
    def generate_pdf(self, uid: int, snapshot: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if profile.get('season'):
                ...
                summary_parts.append(f"Цветотип: {season_names.get(profile['season'], profile['season'])}")
            ...
            if 'makeup' in result and result['makeup']:
                ...
                for section_name, products in result['makeup'].items():
                    if products:
                        ...
                        pdf.multi_cell(0, self.line_height, f"2.{product_count + 1}. {section_title}:")
                        pdf.ln(2)
            ...
            if total_products > 0:
                ...
                stats.append(f"В наличии: {in_stock_products}/{total_products} ({in_stock_percent:.0f}%)")
