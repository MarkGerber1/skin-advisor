    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="d) Периодически, зависит от времени года", callback_data="tightness:d"
            )
        ],
    ]
...
def _kb_sun_reaction() -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="b) Загорает с трудом, появляются веснушки", callback_data="sun:b"
            )
        ],
        [
            InlineKeyboardButton(
                text="c) Мгновенно обгорает, появляется пигментация", callback_data="sun:c"
            )
        ],
...
def _kb_imperfections() -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="a) Расширенные поры и черные точки", callback_data="imperfections:a"
            )
        ],
        ...
        [
            InlineKeyboardButton(
                text="d) Пигментные пятна и покраснения", callback_data="imperfections:d"
            )
        ],
    ]
    return add_home_button(InlineKeyboardMarkup(inline_keyboard=buttons))
...
def _kb_couperose() -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="d) Появляются при перепадах температур", callback_data="couperose:d"
            )
        ],
    ]
    return add_home_button(InlineKeyboardMarkup(inline_keyboard=buttons))
...
def _kb_current_care() -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="a) Минимальный (очищение + увлажнение)", callback_data="care:a"
            )
        ],
        [
            InlineKeyboardButton(
                text="b) Полный (очищение, тоник, сыворотка, крем)", callback_data="care:b"
            )
        ],
        [
            InlineKeyboardButton(
                text="c) Профессиональный с SPF и специальными средствами", callback_data="care:c"
            )
        ],
...
async def q8_desired_effect(cb: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        selector = SelectorV2()
        print(
            f"🔧 DETAILED SKINCARE: Calling selector with profile: skin_type={profile.skin_type}, concerns={[c for c in concerns]}"
        )
        ...
        try:
            ...
            if session:
                ...
                print(
                    f"✅ SAVED profile to FSM coordinator: skin_type={skin_type}, concerns={concerns}"
                )
            else:
        ...
        report_data = ReportData(
            user_profile=profile, skincare_products=skincare_products, makeup_products=[]
        )
        ...
        try:
            ...
            card_files = generate_visual_cards(
                {"user_id": uid, "skin_type": skin_type, "concerns": concerns}, recommendations
            )
        ...
        await cb.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="⬅️ Назад к результатам", callback_data="back:skincare_results"
                        )
                    ],
        ...
        if result and result.get("skincare"):
            ...
            if ANALYTICS_AVAILABLE:
                ...
                if analytics:
                    ...
                    total_products = sum(
                        len(products) for products in skincare_products.values() if products
                    )
            ...
            try:
                ...
                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="🧼 Очищение", callback_data="show_skincare_category:cleanser"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text="💧 Тоник", callback_data="show_skincare_category:toner"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text="🧪 Сыворотка", callback_data="show_skincare_category:serum"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text="🧴 Крем", callback_data="show_skincare_category:moisturizer"
                            )
                        ],
        ...
        await cb.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [
                        InlineKeyboardButton(
                            text="🛍️ Подобрать уход", callback_data="skincare_result:products"
                        )
                    ],
