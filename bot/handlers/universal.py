...
async def home_button_message(message: Message, state: FSMContext) -> None:
    ...
    try:
        print(f"🏠 Home button pressed by user {message.from_user.id if message.from_user else 'Unknown'}")
...
async def emergency_home_keywords(message: Message, state: FSMContext) -> None:
    ...
    try:
        print(f"🚨 Emergency home via keyword '{message.text}' by user {message.from_user.id if message.from_user else 'Unknown'}")
...
async def state_recovery_middleware(message: Message, state: FSMContext) -> bool:
    ...
    try:
        ...
        if current_state and len(state_data.get("error_count", [])) > 3:
            print(f"🚨 User {message.from_user.id if message.from_user else 'Unknown'} seems stuck in state {current_state}")
