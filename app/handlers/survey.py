...
from app.infra.models import SurveyResponse, DiagnosisResult, SurveySession, Recommendation, Download
...
async def start(message: Message, state: FSMContext, session: AsyncSession):
	try:
		...
		await session.execute(insert(SurveySession).values(user_id=message.from_user.id, status="started", answers_json={}))
		await session.commit()
		await ask_question(message, state)
	except Exception:
		logger.exception("start error")
		await message.answer("Сбой при запуске анкеты. Попробуйте еще раз /start")
...
async def resume(message: Message, state: FSMContext):
	...
	if q_index >= len(QUESTIONS):
		await message.answer("Анкета уже завершена. Нажмите Рекомендации для выдачи.")
		return
	await ask_question(message, state)
...
async def cancel(message: Message, state: FSMContext, session: AsyncSession):
	...
	await session.execute(update(SurveySession).where(SurveySession.user_id == message.from_user.id).values(status="aborted"))
	await session.commit()


@router.message(F.text == messages.LEXICON['btn_reset'], StateFilter(None))
async def handle_reset_button_block(message: Message):
	await message.answer("Вы пока не в анкете. Нажмите 🧪 Начать анкету")
...
async def ask_question(message: Message, state: FSMContext):
	...
	if q_index >= len(QUESTIONS):
		...
		await message.answer(messages.LEXICON['survey_finish_prompt'], reply_markup=keyboards.create_confirm_keyboard('action:show_results', messages.LEXICON['btn_show_results']))
		return
	question = QUESTIONS[q_index]
	progress_text = messages.LEXICON['survey_progress'].format(current=q_index + 1, total=len(QUESTIONS))
	await message.answer(f"{progress_text}\n\n{question['text']}", reply_markup=keyboards.create_survey_keyboard(question['id'], question['options'], question['multi']))


@router.callback_query(F.data.startswith('survey:'))
async def handle_survey_answer(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
	try:
		...
		await callback.message.answer("Произошла ошибка. Попробуйте еще раз /resume или /start")
...
async def finalize_survey(message: Message, state: FSMContext, session: AsyncSession):
	try:
		...
		diagnosis_result = DiagnosisResult(survey_response_id=survey_response.id, result_json=diagnosis_data)
		session.add(diagnosis_result)
		await session.execute(update(SurveySession).where(SurveySession.user_id == message.chat.id).values(status="completed", finished_at=func.now(), updated_at=func.now(), answers_json=answers))
		await session.commit()

		catalog = load_catalog()
		rec = build_recommendations(diagnosis_data, catalog)
		rec_row = Recommendation(session_id=survey_response.id, summary_json=rec.get("summary", {}), products_json={"products": rec.get("products", [])}, unavailable_json={"ids": rec.get("unavailable", [])}, replaced_json={"pairs": rec.get("replaced", [])})
		session.add(rec_row)
		await session.commit()
		...
		await message.answer(messages.LEXICON['survey_finish_prompt'])
		await message.answer_document(BufferedInputFile(open(pdf_path, 'rb').read(), filename="skin_advisor_report.pdf"), caption="Ваш персональный план ухода")
	except Exception:
