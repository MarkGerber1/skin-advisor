...
def get_privacy_message() -> str:
    ...
    return LEXICON["privacy_text"].format(
        days=settings.app.data_retention_days, url=settings.app.privacy_policy_url
    )
...
def render_item_text_with_link(item: dict) -> str:
    """Формирует текст с ценой и инлайн-ссылкой 'Купить'. HTML parse_mode."""
    title = f"{item.get('brand','')} {item.get('name','')}".strip()
    stock = item.get("_stock") or {}
    p = stock.get("price")
    price_line = f"≈ {fmt_rub(p)}" if p is not None else "Цена на странице магазина"
    ref = item.get("ref_link") or ""
    buy = f'\n<a href="{ref}">Купить</a>' if ref else ""
    return f"{title}\n{price_line}{buy}"
    ...
    for key, title in titles.items():
        if routine_steps := routines.get(key):
            ...
            for i, step in enumerate(routine_steps):
                ...
                p = stock.get("price")
                price_line = (
                    f"≈ {fmt_rub(p)}" if p is not None else "Цена на странице магазина"
                )
