...
def create_survey_keyboard(
    question_id: str, options: dict, is_multi: bool = False
) -> InlineKeyboardMarkup:
    ...
    for key, value in options.items():
        builder.add(
            InlineKeyboardButton(
                text=value, callback_data=f"survey:{question_id}:{key}"
            )
        )
    ...
    if is_multi:
        builder.add(
            InlineKeyboardButton(
                text=LEXICON["btn_next"], callback_data=f"survey:{question_id}:next"
            )
        )
...
def create_results_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text=LEXICON["btn_get_recommendations"], callback_data="action:get_recs"
        ),
        InlineKeyboardButton(
            text=LEXICON["btn_download_pdf"], callback_data="action:get_pdf"
        ),
    )
    builder.row(
        InlineKeyboardButton(
            text=LEXICON["btn_change_answers"], callback_data="action:reset"
        )
    )
    return builder.as_markup()
...
def create_price_tier_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        ...
        InlineKeyboardButton(
            text=LEXICON["btn_premium"], callback_data="price:premium"
        ),
    )
    return builder.as_markup()
