...
class AffiliateService:
    ...
    def __init__(self):
        ...
        if self.settings is None:
            self.settings = type(
                "MockSettings",
                (),
                {"affiliate_tag": "skincare_bot", "partner_code": "aff_skincare_bot"},
            )()
    ...
    def build_ref_link(
        self, product: Dict[str, Any], campaign: str = "recommendation"
    ) -> Optional[str]:
        ...
        try:
            ...
            print(
                f"🔗 affiliate_link_built: product={product_id}, source={config_key}, priority={self.affiliate_configs[config_key]['priority']}"
            )
            ...
            print(
                f"❌ Error building affiliate link for product {product.get('id', 'unknown')}: {e}"
            )
            import traceback
    ...
    def _detect_source(self, product: Dict[str, Any]) -> Optional[str]:
        ...
        if link:
            ...
            if any(
                ...
                for domain in ["wildberries.ru", "ozon.ru", "yandex.market.ru", "market.yandex.ru"]
            ):
