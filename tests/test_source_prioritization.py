...
class TestSourcePrioritizer:
    ...
    def test_golden_apple_highest_priority(self):
        ...
        for url in golden_apple_urls:
            ...
            assert (
                priority == 1
            ), f"Золотое Яблоко должно иметь приоритет 1, получен {priority} для {url}"
    ...
    def test_russian_official_priority(self):
        ...
        for url in ru_official_urls:
            ...
            assert (
                priority == 2
            ), f"Российские официальные должны иметь приоритет 2, получен {priority} для {url}"
    ...
    def test_russian_marketplace_priority(self):
        ...
        for url in ru_marketplace_urls:
            ...
            assert (
                priority == 3
            ), f"Российские маркетплейсы должны иметь приоритет 3, получен {priority} для {url}"
    ...
    def test_foreign_priority(self):
        ...
        for url in foreign_urls:
            ...
            assert (
                priority == 4
            ), f"Зарубежные должны иметь приоритет 4, получен {priority} для {url}"
    ...
    def test_unknown_source_lowest_priority(self):
        ...
        for url in unknown_urls:
            ...
            assert (
                priority == 999
            ), f"Неизвестные источники должны иметь приоритет 999, получен {priority} для {url}"
    ...
    def test_domain_extraction(self):
        ...
        for url, expected_domain in test_cases:
            ...
            ), f"Для {url} ожидался домен {expected_domain}, получен {actual_domain}"
    ...
    def test_sort_products_by_priority(self):
        ...
        ), f"Неправильная сортировка: ожидалось {expected_order}, получено {actual_order}"
