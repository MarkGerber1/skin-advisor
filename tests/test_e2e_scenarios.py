...
class E2EScenarioGenerator:
    ...
    def __init__(self):
        ...
        self.concerns = [
            ["dryness"], ["acne"], ["aging"], ["pigmentation"],
            ["sensitivity"], ["dryness", "aging"], ["acne", "pigmentation"]
        ]
    ...
    def generate_ambivalent_scenarios(self) -> List[UserScenario]:
        ...
        scenarios.append(UserScenario(
            ...
            profile_data={
                ...
                "skin_type": "dry",
                "concerns": ["dryness", "aging", "sensitivity", "pigmentation"],  # Много проблем
...
class E2EScenarioTester:
        ...
        try:
            ...
            try:
                ...
                if monetization_report.get("monetization_rate", 0) < 95:
                    issues.append(f"Low monetization rate: {monetization_report.get('monetization_rate', 0)}%")
    ...
    def _create_test_catalog(self) -> List:
        ...
        try:
            ...
            makeup_categories = [
                "foundation", "concealer", "corrector", "powder",      # База (4)
                "blush", "bronzer", "contour", "highlighter",         # Лицо (4)
                "eyebrow", "eyeshadow", "eyeliner", "mascara",        # Глаза (4)
                "lipstick", "lip_gloss", "lip_liner"                  # Губы (3)
            ]
            ...
            skincare_categories = [
                "cleanser", "toner", "serum", "moisturizer", "spf",   # Основные (5)
                "exfoliant", "mask"                                   # Дополнительные (2)
            ]
    ...
    def _verify_selection_result(self, result: Dict[str, Any], expected: Dict[str, Any]) -> List[str]:
        ...
        if "makeup" in result:
            ...
            if makeup_count < min_makeup:
                issues.append(f"Insufficient makeup products: {makeup_count} < {min_makeup} (sections: {section_details})")
        else:
    ...
    def run_all_scenarios(self) -> Dict[str, Any]:
        ...
        results_summary = {
            ...
            "basic_success_rate": (sum(1 for r in basic_results if r.passed) / len(basic_results)) * 100 if basic_results else 0,
            "ambivalent_success_rate": (sum(1 for r in ambivalent_results if r.passed) / len(ambivalent_results)) * 100 if ambivalent_results else 0
        }
    ...
    def get_failure_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_issues": sorted(error_categories.items(), key=lambda x: x[1], reverse=True)[:5],
