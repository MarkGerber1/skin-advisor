...
def run_functional_e2e_tests():
    ...
    stability_rate = ((len(functional_scenarios) - critical_failures) / len(functional_scenarios)) * 100
    ...
    pdf_success = sum(1 for r in results if hasattr(r, 'verification_details') and 
                     r.verification_details.get("pdf_generation", "").startswith("SUCCESS"))
    profile_success = sum(1 for r in results if hasattr(r, 'verification_details') and 
                         r.verification_details.get("profile_creation") == "SUCCESS")
    selection_success = sum(1 for r in results if hasattr(r, 'verification_details') and 
                           r.verification_details.get("product_selection") == "SUCCESS")
    
    print(f"Profile Creation: {profile_success}/{len(functional_scenarios)} ({profile_success/len(functional_scenarios)*100:.1f}%)")
    print(f"Product Selection: {selection_success}/{len(functional_scenarios)} ({selection_success/len(functional_scenarios)*100:.1f}%)")
    print(f"PDF Generation: {pdf_success}/{len(functional_scenarios)} ({pdf_success/len(functional_scenarios)*100:.1f}%)")
