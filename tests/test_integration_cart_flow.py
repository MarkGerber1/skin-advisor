...
class TestCartFlowIntegration:
    ...
    def test_end_to_end_cart_flow(self):
        ...
        lipstick_in_cart = next(
            (
                item
                for item in cart_items
                if item.get_composite_key() == "lipstick-mac-001:shade-ruby-woo"
            ),
            None,
        )
    ...
    def test_legacy_cart_migration(self):
        ...
        new_variant = CartItem(
            product_id="old-product-001", variant_id="new-variant", qty=1, brand="Legacy Brand"
        )
