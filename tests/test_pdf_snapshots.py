...
class PDFSnapshotTester:
    ...
    def create_snapshot(
        self, test_name: str, pdf_path: str, test_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        snapshot = {
            ...
            "test_data_hash": hashlib.md5(
                json.dumps(test_data or {}, sort_keys=True).encode()
            ).hexdigest(),
            "expectations": {
    ...
    def verify_snapshot(
        ...
        if current_size < min_size:
            ...
            verification_result["issues"].append(
                f"File size too small: {current_size} < {min_size}"
            )

        if current_size > max_size:
            verification_result["passed"] = False
            verification_result["issues"].append(
                f"File size too large: {current_size} > {max_size}"
            )
        ...
        if expected_hash and current_hash != expected_hash:
            ...
            verification_result["issues"].append(
                f"Content hash changed: {current_hash} != {expected_hash}"
            )
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        results = {"total_tests": len(test_scenarios), "passed": 0, "failed": 0, "test_results": []}
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    def test_generate(uid, snapshot):
                        ...
                        try:
                            ...
                            generator._add_summary_section(
                                pdf, snapshot.get("profile", {}), snapshot.get("type", "test")
                            )
                    ...
                    if pdf_path and os.path.exists(pdf_path):
                        ...
                        verification = self.verify_snapshot(
                            scenario["name"], pdf_path, scenario["data"]
                        )
                ...
                "data": {
                    ...
                    "result": {
                        "makeup": {
                            "base": [
                                {
                                    "name": "Cool Foundation",
                                    "brand": "Brand A",
                                    "category": "foundation",
                                    "price": 2000,
                                    "in_stock": True,
                                    "explain": "для холодного подтона",
                                },
                                {
                                    "name": "Light Concealer",
                                    "brand": "Brand B",
                                    "category": "concealer",
                                    "price": 800,
                                    "in_stock": True,
                                    "explain": "маскирует несовершенства",
                                },
                            ],
                            "eyes": [
                                {
