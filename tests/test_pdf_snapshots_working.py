...
class WorkingPDFSnapshotTester:
    ...
    def create_snapshot(self, test_name: str, pdf_path: str) -> Dict[str, Any]:
        ...
        print(f"✅ Created snapshot '{test_name}': {signature['file_size']} bytes, {signature['size_category']}")
        return snapshot
    ...
    def verify_snapshot(self, test_name: str, pdf_path: str) -> Dict[str, Any]:
        ...
        if expected_size > 0:
            ...
            if size_diff > size_tolerance:
                result["passed"] = False
                result["issues"].append(f"Size difference too large: {size_diff:.1%} > {size_tolerance:.1%}")
        ...
        if not tolerances.get("allow_size_category_change", False):
            ...
            if expected_category and current_category != expected_category:
                result["passed"] = False
                result["issues"].append(f"Size category changed: {current_category} != {expected_category}")
